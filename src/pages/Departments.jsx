...
export default function Departments() {
  ...
  // Fetch category fees from Supabase
  const fetchCategoryFees = async () => {
    try {
      ...
      // Group fees by academic_year, group, course, and semester
      ...
      data.forEach(item => {
        ...
        if (!feesMap.has(key)) {
          feesMap.set(key, {
            id: item.id, // Keep the first ID for reference
            feeCategories: [],
            amount: item.amount, // All amounts should be the same for the same group
        ...
        // Add the fee category to the group
        if (item.fee_cat) {
  ...
  const submitAllCategories = async () => {
    ...
    try {
      ...
      const feesBySemester = {};
      
      categoryFees.forEach(fee => {
        const key = `${fee.academic_year}_${fee.group}_${fee.course_code}_${fee.semester}`;
        if (!feesBySemester[key]) {
          feesBySemester[key] = {
            academic_year: fee.academic_year,
            group: fee.group,
            course: fee.course_code,
            semester: fee.semester,
            categories: {}
          };
        }
        feesBySemester[key].categories[fee.name] = fee.amount;
      });

      // Process each semester
      for (const semesterKey in feesBySemester) {
        const semester = feesBySemester[semesterKey];
        
        // First, delete existing fees for this semester
        await supabase
          .from('fee_structure')
          .delete()
          .eq('academic_year', String(semester.academic_year))
          .eq('group', semester.group)
          .eq('course', semester.course)
          .eq('semester', semester.semester);

        // Prepare data for insertion
        // Prepare data for insertion (ONE ROW ONLY)
        const insertData = {
          academic_year: String(semester.academic_year),
          group: semester.group,
          course: semester.course,
          semester: semester.semester,

          // Combine all fee categories into ONE string
          fee_cat: Object.keys(semester.categories).join(", "),

          // Common amount (because all categories share the same amount)
          amount: parseFloat(semester.commonAmount),
      ...
      alert("Fee Categories Saved Successfully!");
    } catch (error) {
